...
export default function HomeSocialProof() {
  ...
  return (
    <section ref={containerRef} className="bg-forest-lift py-24 md:py-32 overflow-hidden">

      {/* Ticker */}
      <div className="overflow-hidden border-y border-white/10 py-4 mb-20">
        <div className="flex whitespace-nowrap">
          <div ref={tickerRef} className="flex shrink-0 gap-12 pr-12">
            {IMPACT.ticker.map((item, i) => (
              <div key={i} className="flex items-center gap-3">
                <span className="font-mono text-gold-muted text-xs tracking-wider font-bold">{item.stat}</span>
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-10">
        ...
        <div className="proof-header flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6 mb-14">
          <div>
            <div className="inline-flex items-center gap-2 border border-gold/25 bg-gold/8 text-gold-muted rounded-pill px-4 py-1.5 mb-5">
            ...
            <p className="font-sans text-white/45 text-base mt-2">{IMPACT.subhead}</p>
          </div>
          <Link to="/impact"
            className="font-sans text-sm text-gold-muted hover:text-gold border border-gold/25 hover:border-gold/50 px-5 py-2.5 rounded-pill transition-all duration-200 shrink-0">
            See all results →
          </Link>
        </div>
        ...
        <div className="proof-grid grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="proof-featured lg:col-span-2 relative bg-forest-card rounded-4xl p-8 md:p-10 border border-white/10 overflow-hidden flex flex-col justify-between shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                <div className="w-11 h-11 rounded-full bg-gold flex items-center justify-center text-white font-bold text-sm shrink-0">
                  {featured.name.split(' ').map(n => n[0]).join('')}
                </div>
                <div>
                  <p className="font-sans font-bold text-white text-sm">{featured.name}</p>
                  <p className="font-sans text-white/40 text-xs">{featured.role}</p>
                </div>
              </div>
              <div className="flex gap-6">
                {featured.stats?.map((s, i) => (
                  <div key={i} className="text-center">
                    ...
                    <div className="font-mono text-white/30 text-[10px] tracking-wider">{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div key={i} className={`proof-card relative rounded-4xl p-6 border overflow-hidden flex-1 ${
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${
                    card.isCohort ? 'bg-amber text-midnight' : 'bg-gold/20 text-gold-muted'
                  }`}>
