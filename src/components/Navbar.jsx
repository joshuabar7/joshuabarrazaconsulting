  ...
  return (
    <nav ref={navRef} className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-5 px-4">
      ...
      >
        ...
        {/* Nav tabs - right side (desktop) */}
        <div className="hidden md:flex items-center gap-1">
          {nav.links.map(({ label, href }) => {
            const isCTA = label === nav.cta
            if (isCTA) return (
              ...
              return (
                <a key={label} href={href} onClick={() => setMenuOpen(false)}
                  className={isCTA
                    ...
                    : 'font-sans text-sm font-medium text-slate-mid hover:text-jet px-4 py-3 rounded-2xl hover:bg-off-white transition-all'
                  }>
                  {label}
                </a>
              )
