  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-12">
      ...
      <button
        ...
        className={`relative text-left rounded-4xl p-7 border-2 overflow-hidden transition-all duration-300 ${
          activeTrack === 'launch'
            ...
            : 'bg-forest-lift border-white/10 hover:border-amber/40'
        }`}
      >
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          ...
          {activeTrack === 'launch' && (
            <div className="bg-amber text-white font-mono text-[10px] font-bold tracking-wider uppercase rounded-pill px-3 py-1">
              Selected
            </div>
          )}
        </div>

        <h3 className="font-sans font-extrabold text-white text-lg mb-1">The Launch Program</h3>
        <p className="font-mono text-xs text-amber-muted mb-3">For high school + college students</p>
        <p className="font-sans text-sm text-white/55 leading-relaxed mb-4">
      ...
      >
        ...
        <p className={`font-sans text-sm leading-relaxed mb-4 ${activeTrack === 'premium' ? 'text-white/50' : 'text-white/55'}`}>
